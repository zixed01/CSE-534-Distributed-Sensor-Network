...
def run_setup():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        print(f"Cluster head: {cluster_head_hostname}: "
...
# this function is called by the cluster_head when the RTT to the sink is
# outside the threshold to request a RERUN_SETUP by the sink
def request_rerun_setup():
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        sock.settimeout(2)
        message = f"RERUN_SETUP:{CLUSTER_ID}"
        try:
            sock.sendto(message.encode(), (SINK_IP, SINK_COMMAND_PORT))
            print("RERUN_SETUP request sent to sink")

        except socket.timeout:
            print("RERUN_SETUP request timed out")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        while True:
            ...
            try:
                ...
                print(f"Received ACK from sink. RTT: {rtt * 1000:.2f} ms")
            ...
            if rtt > AVG_RTT * 1.25:
                print(f"Current RTT above threshold. Sending RERUN_SETUP"
                    f" request now.")
                request_rerun_setup()
            elif rtt < AVG_RTT * 0.50:
                print(f"Current RTT below threshold. Sending RERUN_SETUP"
                    f" request now.")
                request_rerun_setup()
