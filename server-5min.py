...
# periodic cluster head selection (5 min for this instance)
def periodic_cluster_head_selection(interval=300):
...
# notify all nodes to rerun the setup phase
def notify_nodes_to_rerun_setup():
    print("Notifying all nodes to rerun setup process")
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
...
def main():
    ...
    threading.Thread(target=periodic_cluster_head_selection, args=(300,),
        daemon=True).start()
